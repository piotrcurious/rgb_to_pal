// ESP32 PAL composite video encoder with 8-color RGB input and full chroma modulation // Author: ChatGPT (optimized with inline assembly)
...
void IRAM_ATTR sampleRGB8(uint8_t &color) { // Inline assembly version for faster GPIO sampling uint32_t val; asm volatile ( "rsr %0, GPIO_IN_REG" : "=r"(val) ); uint8_t r = (val >> PIN_R) & 1; uint8_t g = (val >> PIN_G) & 1; uint8_t b = (val >> PIN_B) & 1; color = (r << 2) | (g << 1) | b; }
...
inline uint8_t IRAM_ATTR fastLUT(uint8_t col, uint16_t phase) { uint32_t offset = ((uint32_t)col << 8) | (phase & 0xFF); return *((volatile uint8_t *)(&lut[0][0]) + offset); }
...
for (int px = 0; px < PIXELS_PER_LINE; px++) { uint8_t col; sampleRGB8(col); float v = rgb2yuv[col].v; float vavg = 0.5f * (v + prevV[px]); prevV[px] = v; for (int s = 0; s < SAMPLES_PER_PIXEL; s++, idx++) { out[idx] = fastLUT(col, phaseIndex); phaseIndex = (phaseIndex + 1) & (LUT_SIZE - 1); } }
